...
from ( 
	...
	from waterworks_billing_period bs 
		INNER JOIN waterworks_bill wb ON wb.periodid=bs.objid 
		inner join waterworks_consumption c on (c.objid = wb.consumptionid and wb.state='POSTED' and c.hold=0)
		inner join waterworks_account_info a on a.objid = wb.acctinfoid 
		inner join waterworks_subarea sa on sa.objid = a.subareaid 
