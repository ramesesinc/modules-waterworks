[getBarangays]
select distinct 
	wz.barangay_objid as objid, wz.barangay_name as name 
from waterworks_subarea wz 
where wz.barangay_objid is not null 
order by wz.barangay_name 

[getZones]
select 
	wz.objid, wz.code, wz.description, 
	wz.barangay_objid, wz.barangay_name 
from 
	waterworks_subarea wz 
where wz.barangay_objid is not null 
order by wz.code 
...
from ( 
	...
	from waterworks_account acct
	    ...
	    INNER JOIN waterworks_subarea wz ON wa.subareaid = wz.objid 
		LEFT JOIN waterworks_meter wm on wa.meterid = wm.objid 
	where acct.state = 'ACTIVE' 
		and wz.barangay_objid is not null 
) v  
